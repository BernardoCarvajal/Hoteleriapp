...
export class TicketComponent implements OnInit {
  ...
  cargarTicket() {
    if (!this.reserva || !this.reserva.id) {
      this.error = 'No se pudo cargar el ticket: información de reserva incompleta';
      this.cargando = false;
      return;
    }
  ...
  generarQR() {
    ...
    const ticketData = {
      ...
      fechaInicio: this.formatearFecha(this.reserva.fecha_inicio),
      fechaFin: this.formatearFecha(this.reserva.fecha_fin),
      habitaciones: this.reserva.habitaciones,
      total: this.reserva.total
    };
    
    // URL para acceder al ticket directamente
    const ticketUrl = `${environment.apiUrl.replace('/api', '')}/ticket/${this.reserva.id}`;
    
    // Generamos el QR usando una API
    this.qrDataUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(ticketUrl)}`;
  }
  ...
  descargarTicket() {
    ...
    setTimeout(() => {
      ...
      const ticketElement = document.querySelector('.ticket-content') as HTMLElement;
      ...
      html2canvas(ticketElement, {
        ...
        pdf.text('Este documento es un comprobante oficial de su reserva en Hoteleriapp', 105, 285, { align: 'center' });
        pdf.text(`Generado el ${this.getCurrentDate()}`, 105, 290, { align: 'center' });
        
        // Guardar PDF
        pdf.save(`Ticket_Reserva_${this.reserva.codigo_reserva}.pdf`);
        this.generandoPDF = false;
      }).catch(err => {
